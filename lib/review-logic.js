...
(function (root, factory) {
  ...
  function correctionStatusLabel(s) {
    ...
    // 'superseded' (2026-08-21) : demande fermee comme DOUBLON d'une autre, deja en
    // traitement. Elle etait fermee en 'done' auparavant — donc affichee « ✅ Corrige »
    // alors que rien n'avait ete fait pour elle. Et si l'originale mourait ensuite en
    // needs_review, les deux etaient perdues pendant que l'ecran annoncait un succes.
    if (s.status === 'superseded') {
      return { t: '↪ Doublon — c\'est l\'autre demande qui fait le travail', c: 'pending' };
    }
    if (s.status === 'done') {
      var wt = '';
      if (s.completed_at) {
  ...
  // ── Statut d'un flag de phrase (crayon) — meme vocabulaire visuel ──
  // que correctionStatusLabel : Hela lit les DEUX circuits avec les memes codes.
  function sentenceFlagStatusLabel(f) {
    if (!f) return { t: '', c: '' };
    if (f.applied) {
      var wt = '';
      if (f.applied_at) {
        var w = new Date(f.applied_at);
        var p = function (n) { return String(n).padStart(2, '0'); };
        wt = ' le ' + p(w.getDate()) + '/' + p(w.getMonth() + 1) + ' à ' + p(w.getHours()) + 'h' + p(w.getMinutes());
      }
      return { t: '✅ Phrase corrigée' + wt, c: 'done' };
    }
    if (f.auto_status === 'skipped')
      return { t: '⚠ Pas appliquée automatiquement' + (f.skip_reason ? ' — ' + f.skip_reason : ''), c: 'error' };
    // 'dismissed' (triage source-aware) tombait dans le libellé par défaut
    // « En file — en traitement » : Héla a resoumis la même correction 3 jours
    // de suite (X-Card, 19-21/08) en attendant une réponse déjà rendue en
    // silence. Un rejet doit se LIRE, avec sa raison.
    if (f.auto_status === 'dismissed')
      return { t: '✋ Non retenue' + (f.skip_reason ? ' — ' + f.skip_reason : ' — la source est déjà correcte'), c: 'error' };
    if (f.auto_status === 'processing') return { t: '⚙️ En cours…', c: 'processing' };
    if (f.auto_status === 'converted') return { t: '🔁 Voix à refaire — envoyée au re-roll', c: 'pending' };
    if (f.auto_status === 'superseded') return { t: '↪ Remplacée par un renvoi', c: 'pending' };
    if (f.auto_status === 'obsolete') return { t: '💤 Obsolète (voix déjà régénérée depuis)', c: 'pending' };
    if (f.auto_status === 'backlog') return { t: '⏳ En attente (ancienne file)', c: 'pending' };
    return { t: '⏳ En file — en traitement', c: 'pending' };
  }
  ...
  function buildCorrection(opts) {
    ...
    // MOT NU (audit 2026-07-02) : d.word arrive brut des timestamps, souvent avec la
    // ponctuation collee ("répandue."). Une note "répandue. → rependue" est
    // introuvable dans la source -> le worker echouait 5 fois puis abandonnait.
    ...
    if (cat === 'typo') {
      ...
      // Mot identique une fois la ponctuation retiree ("répandue." corrige en "répandue") :
      // le texte source est deja bon — il n'y a rien a corriger, refuser proprement.
      if (value === word)
        return { ok: false, error: 'C’est déjà ce mot-là dans le texte. Si c’est la voix qui sonne mal, choisis « prononciation ».' };
  ...
  // Approbation PERSISTANTE : une approbation explicite (confirmApprove) met true ; une autosave
  // (explicitApprove=false) NE DOIT PAS dés-approuver -> on garde l'état courant. Sinon, approuver
  // puis toucher quoi que ce soit remettait approved=false dans voice_reviews (bug 2026-06-26).
  function approvedToPersist(explicitApprove, currentApproved) {
    return explicitApprove === true || !!currentApproved;
  }
  ...
  return {
    ...
    approvedToPersist: approvedToPersist,
    ...
    sentenceFlagStatusLabel: sentenceFlagStatusLabel,
