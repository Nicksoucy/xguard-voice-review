...
(function (root, factory) {
  ...
  // Retourne show=true si le bandeau d'alerte doit s'afficher (>15 min sans battement, ou erreur).
  function healthState(health, nowMs) {
    if (!health || !health.last_heartbeat) return { show: false };
    var ageMin = (nowMs - new Date(health.last_heartbeat).getTime()) / 60000;
    var error = health.status === 'error';
    if (ageMin <= 15 && !error) return { show: false, ageMin: ageMin, error: error };
    var pending = (health.next_jobs && health.next_jobs.pending) || 0;
    return { show: true, error: error, ageMin: ageMin, pending: pending, version: health.version };
  }
