...
(function (root, factory) {
  ...
  // Nombre de lecons dont la VOIX est reglee : le total moins celles qui attendent
  // encore une revue ou une re-ecoute.
  //
  // POURQUOI CETTE FONCTION EXISTE (6 aout 2026)
  // Le cockpit affichait « Voix t/t » en dur — le numerateur ETAIT le denominateur,
  // donc toujours 100 %. Hela voyait « Voix 17/17 » juste au-dessus d'un badge
  // « 1 a faire » : deux chiffres qui se contredisent sur la meme carte. Le calcul
  // juste dormait ici, utilise seulement pour choisir un libelle de phase.
  function voiceDoneCount(s) {
    return sumStages(s) - (s.voice_review || 0) - (s.voice_recheck || 0);
  }

  // Phase d'une formation d'apres ses compteurs : Pret LMS / Phase video / Phase voix.
  function coursePhase(s) {
    var t = sumStages(s);
    var voiceDone = voiceDoneCount(s);
  ...
  // Etat PAR MACHINE des workers (audit 2026-07-02). Les heartbeats sont desormais
  // une ligne par machine et par worker (id "xguard-correction@HOST", "xguard-sflags@HOST"...).
  // Avant : Mac et Nitro s'ecrasaient mutuellement sur une ligne unique -> version en
  // flip-flop et panne invisible (la ligne 'nitro' est restee morte 5 jours sans alerte).
  // Niveaux par machine :
  //   'error' : status error OU stale-version (code perime) — toujours visible.
  //   'alert' : >30 min sans battement ET du travail en attente (travail coince).
  //   'idle'  : froid depuis longtemps mais rien en attente (Mac endormi = normal).
  //   'ok'    : battement recent.
  function machinesState(rows, nowMs, pendingNow) {
    var machines = (rows || [])
      .filter(function (r) { return r && r.id && r.id.indexOf('@') !== -1 && r.last_heartbeat; })
      .map(function (r) {
        var parts = r.id.split('@');
        var ageMin = (nowMs - new Date(r.last_heartbeat).getTime()) / 60000;
        var hbPending = (r.next_jobs && r.next_jobs.pending) || 0;
        var pending = Math.max(pendingNow || 0, hbPending);
        var level = 'ok';
        if (r.status === 'error' || r.status === 'stale-version') level = 'error';
        else if (ageMin > 30 && pending > 0 && parts[0] === 'xguard-correction') level = 'alert';
        else if (ageMin > 30) level = 'idle';
        return {
          id: r.id,
          worker: parts[0].replace('xguard-', ''),
          host: parts[1],
          version: r.version || '?',
          status: r.status,
          ageMin: ageMin,
    ...
    // Version de reference = celle de la machine au battement le plus recent parmi
    // les vivantes ; toute machine vivante avec une AUTRE version = "perimee".
    var fresh = machines.filter(function (m) { return m.ageMin <= 30 && m.version && m.version !== '?'; })
      .sort(function (a, b) { return a.ageMin - b.ageMin; });
    var ref = fresh.length ? fresh[0].version : null;
    machines.forEach(function (m) {
      m.staleVersion = !!(ref && m.ageMin <= 30 && m.version !== '?' && m.version !== ref);
      if (m.staleVersion && m.level === 'ok') m.level = 'error';
    });
    return { machines: machines, show: machines.length > 0 };
  }
